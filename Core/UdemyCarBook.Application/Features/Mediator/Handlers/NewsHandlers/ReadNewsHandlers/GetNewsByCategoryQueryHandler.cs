...
{
    public class GetNewsByCategoryQueryHandler : IRequestHandler<GetNewsByCategoryQuery, List<GetNewsQueryResult>>
    {
        private readonly INewsRepository _repository;

        public GetNewsByCategoryQueryHandler(INewsRepository repository)
        {
            _repository = repository;
        }

        public async Task<List<GetNewsQueryResult>> Handle(GetNewsByCategoryQuery request, CancellationToken cancellationToken)
        {
            var news = await _repository.GetAllWithDetailsAsync();
            var categoryNews = news.Where(x => x.CategoryId == request.CategoryId).ToList();

            return categoryNews.Select(x => new GetNewsQueryResult
            {
                Id = x.Id,
                Title = x.Title,
                Content = x.Content,
